...
const createAxiosInstance = (baseUrl: string, tokenKey: string): AxiosInstance => {
    ...
    instance.interceptors.request.use(
        async function (config: InternalAxiosRequestConfig) {
            const token_camboquick = CookieCore.get(KEY_COOKIES.WEBSITE)
            const language = CookieCore.get("language")

            if (token_camboquick !== undefined) {
                config.headers["Authorization"] = "Bearer " + token_camboquick;
            }
