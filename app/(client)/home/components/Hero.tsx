...
const Hero = () => {
  ...
  const goToImage = (targetIndex: number) => {
    if (isAnimating || targetIndex === currentImageIndex || slidesLength === 0)
      return; // Đang animate, không có dữ liệu hoặc đã ở vị trí đó thì không làm gì
    setIsAnimating(true);
        ...
        <div className="min-h-[40vh] lg:min-h-[60vh] flex flex-col-reverse lg:flex-row gap-9 xl:gap-20 2xl:gap-24 items-center w-full h-full">
          <div className="flex flex-col gap-9 w-full lg:w-[50%] xl:w-[40%]">
            <div className="flex flex-col gap-5">
              ...
              <p className="text-base-default w-full lg:w-[90%] text-center lg:text-left font-semibold text-light-900">
                FOSO không chỉ thiết kế app, mà còn giúp bạn kể câu chuyện
                thương hiệu qua từng cú chạm – độc đáo, tinh tế và đậm chất
                riêng.
              </p>
            </div>
            <div className="flex flex-col xxs:flex-row items-center justify-center lg:justify-start gap-3 w-full">
              <ButtonAnimationNew
                ...
                style={{
                  ...
                  background:
                    "linear-gradient(90deg, #F3654A 0%, #FFB9AC 100%)",
          ...
          {slidesLength > 0 ? (
            <div className="relative w-full lg:w-[60%] h-full">
              <div className="relative w-1/2 ml-[37.5%] mr-[12.5%]">
                ...
                <div
                  className="hidden absolute top-1/2 -translate-y-1/2 -left-[30%] size-[50px] lg:size-[44px] 2xl:size-[60px] rounded-full bg-[#E96C6C]/60 hover:bg-[#E96C6C] cursor-pointer transition-all duration-500 lg:flex items-center justify-center p-3.5 2xl:p-4 border border-white/60"
                  onClick={prevImage}
                >
                  <Image
                    src={IMAGES.arrowRightOrange}
                    alt="arrowRightOrange"
                    width={1000}
                    height={1000}
                    className="size-full rotate-180 shrink-0 object-contain"
                  />
                </div>
                <div
                  className="hidden absolute top-1/2 -translate-y-1/2 -right-[30%] size-[50px] lg:size-[44px] 2xl:size-[60px] rounded-full bg-[#E96C6C]/60 hover:bg-[#E96C6C] cursor-pointer transition-all duration-500 lg:flex items-center justify-center p-3.5 2xl:p-4 border border-white/60"
                  onClick={nextImage}
                >
                  <Image
                    src={IMAGES.arrowRightOrange}
                ...
                {slidesLength > 0 && (
                  <div className="hidden lg:flex absolute xl:-bottom-[13%] lg:-bottom-[8%] left-1/2 -translate-x-1/2 gap-2 items-center">
                    {slides.map((_, index) => (
                      <div
                        key={index}
                        className={`w-2.5 h-2.5 lg:w-3 lg:h-3 2xl:w-4 2xl:h-4 rounded-full transition-all duration-300 cursor-pointer ${
              ...
              <div
                className="absolute bottom-0 left-0 p-3 xl:p-4 pb-6 bg-orange-100 rounded-[20px] xl:rounded-[30px] w-[65%] xl:w-[55%] 2xl:w-1/2 h-fit"
                style={{
                  boxShadow:
                    "0px 1px 3px -1px #0000004D, 0px 6px 10px -1px #32325D40",
                }}
              >
                {isLoading || isError ? (
                  <>
                      ...
                      <div className="flex gap-2 items-center p-2 pr-4 bg-gradient-to-r from-[#F3654A] to-[#FFB9AC] rounded-full w-full h-fit">
                        ...
                        <div className="flex flex-col">
                          <p className="text-title font-semibold text-white">
                            {slides[currentImageIndex].name}
                          </p>
                          <p className="text-sxs-default text-white">
                            {slides[currentImageIndex].position}
                          </p>
                        </div>
                      </div>
                      <AnimatedTyping
